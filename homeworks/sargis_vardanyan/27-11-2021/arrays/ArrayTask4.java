package arrays;

import java.util.Scanner;

/*
   characters is an array of chars with the size of n.
   Task: Make an array which members are the same as characters, but 
   after every 'f' symbols there is on more 'f'.
*/

public class ArrayTask4 {

    public static void main(String [] args) {
    
        System.out.println("Enter Characters's size: ");
        Scanner scanner = new Scanner(System.in);
        int n = scanner.nextInt();
        char [] characters = new char [n];
        System.out.println("Enter " + n + " chars: ");
        scanner = new Scanner(System.in);
        
        int countOfF = 0;
        
        for (int i = 0; i < characters.length; i++) {
            characters[i] = scanner.nextLine().charAt(0);
            if(characters[i] == 'f') {
                countOfF++;
            }
        }
        
        char [] newArray = new char [n + countOfF];
        
        for (int i = 0, j = 0; i < newArray.length; i++) {
            if(characters[j] == 'f') {
                newArray[i] = characters[j];
                newArray[i + 1] = 'f';
                j++;
                i++;
                continue;
            }

            newArray[i] = characters[j++];
        }
        
        System.out.print("newArray: ");
        
        for (int i = 0; i < newArray.length; i++) {
            System.out.print(newArray[i] + ", ");
        }
        System.out.println();
        
    
    
    }

}









